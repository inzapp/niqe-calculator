...
def compute_image_mscn_transform(image, C=1, avg_window=None, extend_mode='constant'):
    ...
    mu_image = np.zeros(image.shape, dtype=np.float32)
    var_image = np.zeros(image.shape, dtype=np.float32)
...
def niqe(img):
    ...
    # TODO: memoize
    params = scipy.io.loadmat(join(module_path, 'params.mat'))
    ...
    assert M > (patch_size*2+1), f"niqe called with small frame size, requires > {patch_size * 2}x{patch_size * 2} resolution video using current training parameters"
    assert N > (patch_size*2+1), f"niqe called with small frame size, requires > {patch_size * 2}x{patch_size * 2} resolution video using current training parameters"
